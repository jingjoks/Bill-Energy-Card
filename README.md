...
- **เลือกภาษาได้ทั้งไทย/อังกฤษ** สลับได้จากปุ่มในแถวตั้งค่าด้านล่างของตัวการ์ดเองหรือในหน้าตั้งค่า ข้อความทั้งการ์ดเปลี่ยนตามทันที
...
- **(ไม่บังคับ) ส่วนแสดงกำลังไฟเรียลไทม์** — ใส่ `grid_power_entity`/`load_power_entity` (และ voltage/current ถ้ามี) จะมี % ใช้จากโซลาร์ตอนนี้, กำลังไฟ/แรงดัน/กระแสสด, กราฟเส้นกำลังไฟ (kW) ย้อนหลังโผล่มาด้านบนของการ์ด ไม่ตั้งค่าก็ไม่มีอะไรเปลี่ยนจากเดิม
...
อยากเปิดส่วนแสดงกำลังไฟเรียลไทม์ด้านบนด้วย เพิ่ม 2 ตัวนี้เข้าไป (ดูตัวอย่างเต็มใน [`examples/with-live-meter.yaml`](examples/with-live-meter.yaml)):
...
| `language` | `th` | ภาษาที่แสดงในการ์ด: `th` หรือ `en` (สลับจากปุ่มในแถวตั้งค่าด้านล่างของตัวการ์ดก็ได้ ไม่ต้องแก้ YAML) |
...
| `rate_scheme` | `tiered` | รูปแบบคำนวณค่าไฟ: `tiered` (ขั้นบันได ปกติ) หรือ `tou` (ตามช่วงเวลา) |
| `tou_on_peak_rate` | `4.1025` | อัตรา On-Peak (บาท/หน่วย) — ใช้เมื่อ `rate_scheme: tou` |
| `tou_off_peak_rate` | `2.5849` | อัตรา Off-Peak (บาท/หน่วย) |
| `tou_on_peak_start_hour` / `tou_on_peak_end_hour` | `9` / `22` | ช่วงเวลา On-Peak (จันทร์-ศุกร์เท่านั้น เสาร์-อาทิตย์เป็น Off-Peak ทั้งวันเสมอ) |
| `tou_service_charge` | `38.22` | ค่าบริการสำหรับมิเตอร์ TOU (บาท/เดือน) — แยกจาก `service_charge` ของมิเตอร์ปกติ |
...
| `cutoff_day_entity` | *(ไม่บังคับ)* | `entity_id` ของ `input_number` ที่เก็บวันตัดรอบ — ถ้าตั้งไว้ จะมีช่องแก้ค่านี้โผล่มาในแถวตั้งค่าด่วนบนตัวการ์ดด้วย |
| `cutoff_hour_entity` | *(ไม่บังคับ)* | `entity_id` ของ `input_number` ที่เก็บชั่วโมงตัดรอบ — เช่นเดียวกับด้านบน |
| `grid_power_entity` | *(ไม่บังคับ)* | เซ็นเซอร์กำลังไฟกริดแบบเรียลไทม์ (W) — ตั้งคู่กับ `load_power_entity` เพื่อเปิดส่วนแสดงผลสด |
| `load_power_entity` | *(ไม่บังคับ)* | เซ็นเซอร์กำลังไฟโหลดแบบเรียลไทม์ (W) |
| `grid_voltage_entity` / `load_voltage_entity` | *(ไม่บังคับ)* | เซ็นเซอร์แรงดัน แสดงเป็นรายละเอียดรองในส่วนกำลังไฟเรียลไทม์ |
| `grid_current_entity` / `load_current_entity` | *(ไม่บังคับ)* | เซ็นเซอร์กระแส แสดงเป็นรายละเอียดรองในส่วนกำลังไฟเรียลไทม์ |
| `live_sparkline_hours` | `6` | จำนวนชั่วโมงย้อนหลังที่แสดงในกราฟเส้นกำลังไฟ |
...
> ⚠️ **โหมด TOU**: อัตราเริ่มต้นเป็นค่าอ้างอิงทั่วไป ต้องตรวจสอบ/แก้ให้ตรงกับใบแจ้งหนี้ TOU จริงของคุณ (อัตราต่างกันตามประเภทผู้ใช้ไฟ) และ**ไม่ได้รวมวันหยุดราชการ** (วันหยุดราชการที่ตรงกับวันธรรมดาจะถูกคิดเป็นวันธรรมดาแทน เพราะปฏิทินวันหยุดเปลี่ยนทุกปี) ตัวเลขอาจสูงกว่าบิลจริงเล็กน้อยถ้ามีวันหยุดราชการในรอบนั้น — วันเสาร์-อาทิตย์คิดเป็น Off-Peak ทั้งวันให้แล้วอัตโนมัติ

> 💡 `cutoff_day_entity` / `cutoff_hour_entity` ใช้คู่กับวิธีตั้งรอบบิลแบบ Utility Meter + Automation ใน [`SETUP_GUIDE.md`](SETUP_GUIDE.md) — ถ้าตั้ง 2 ค่านี้ไว้ ช่องแก้วัน/ชั่วโมงตัดรอบจะโผล่มาในแถวตั้งค่าด่วนบนตัวการ์ด แก้แล้วบันทึกจริงทันที (เรียก `input_number.set_value` ตรง ไม่ใช่พรีวิวสดแบบ Ft/ค่าบริการ/VAT)
